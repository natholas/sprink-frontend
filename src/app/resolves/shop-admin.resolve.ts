...
export class ShopAdminResolve implements Resolve<any> {
  ...
  async resolve(route: ActivatedRouteSnapshot) {
    ...
    if (!this.state.user) this.router.navigate(['continue'])
      ...
      if (!shop) this.router.navigate(['dashboard'])
        ...
        let nextNeededStep = await this.user.getNextStep('shop-admin', shop)
        if (nextNeededStep) {
          this.router.navigate(nextNeededStep.path, { queryParams: nextNeededStep.queryParams })
        } else {
          let orders = await this.orders.getShopOrders(shop._id, ['ready', 'pickedup', 'atshop'])
          return [shop, orders]
        }
      }
    }
